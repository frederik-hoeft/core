...
public ref struct ConcurrentBitmap56
{
    ...
    internal const int MAX_CAPACITY = 56;
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNotInRange(index, 0, 55, nameof(index));
        ulong mask = 1uL << index;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ConcurrentBitmap56State FullState(int capacity)
    {
        ulong mask = GetFullMask(capacity);
        return Unsafe.As<ulong, ConcurrentBitmap56State>(ref mask);
    }
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNotInRange(capacity, 1, 56, nameof(capacity));
        ref ulong target = ref AsUlongPointer(ref state);
        ulong oldState, newState;
        do
        {
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNotInRange(index, 0, 55, nameof(index));
        ulong initialBitState = ((AtomicBoolean)isInitiallySet).As64BitMask();
        ref ulong target = ref AsUlongPointer(ref state);
        ulong oldState, newState;
        do
        {
    ...
    public static void RemoveBitAt(ref ConcurrentBitmap56State state, int index)
    {
        ...
        do
        {
            ...
            ulong withUnsetBit = UpdateBitUnsafe(oldState, index, isSet: AtomicBoolean.FALSE);
            ulong splitMask = (1uL << index) - 1;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong UpdateBitUnsafe(ulong state, int index, AtomicBoolean isSet)
    {
